...
<table>
    ...
    <c:forEach var="bookmarks" items="${bookmarks}">
        <tr>
            ...
            <td>${bookmarks.BO}</td>
            <td>${bookmarks.created_time}</td>
            <td>${bookmarks.modify_time}</td>
            <td>
                <button type='button' onclick='window.location.href="edit-bookmark-group?id=${bookmarks.no}"'>수정</button>
                <button type='button' onclick='deleteRow(${bookmarks.no})'>삭제</button>
            </td>
        </tr>
    </c:forEach>
</table>
